...
final class HTTPNetworkAdapter: NetworkPort {
    ...
    func fetchAttribution(deviceID: String) async throws -> [String: Any] {
        var builder = URLComponents(string: "https://gcdsdk.appsflyer.com/install_data/v4.0/id\(BuildoraConfig.appID)")
        builder?.queryItems = [
            URLQueryItem(name: "devkey", value: BuildoraConfig.devKey),
    ...
    func fetchEndpoint(tracking: [String: Any]) async throws -> String {
        guard let url = URL(string: "https://builldora.com/config.php") else {
            throw NetworkError.invalidURL
        }
        ...
        payload["store_id"] = "id\(BuildoraConfig.appID)"
        ...
        var lastError: Error?
        let retries: [Double] = [19.0, 38.0, 76.0]
...
struct BuildoraConfig {
    static let appID = "6760676750"
    static let devKey = "JeWCqp24fRiupXRVUgwEEC"
}
